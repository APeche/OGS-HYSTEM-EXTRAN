...
void CFiniteElementStd::Config()
{
	...
	//store old pressures in vector
	//--------------------------------------------------------------------
	/*OLD leakage function pressure harvesting*/
	////////if ((((this)->pcs)->_problem)->OGSHE == 0 && (((this)->pcs)->_problem)->OGSHECouplingBC == "$LEAKAGE_FUNCTION")
	////////{
	////////	if (index == 0)	//AP erase old vector when starting with first matrix element
	////////	{
	////////		ST_PressureNodeIDs.erase(ST_PressureNodeIDs.begin(), ST_PressureNodeIDs.end());
	////////		ST_Pressures.erase(ST_Pressures.begin(), ST_Pressures.end());
	////////	}
	////////	if (std::find((((this)->pcs)->_problem)->OGSHE_FEMMshElementForLF.begin(), (((this)->pcs)->_problem)->OGSHE_FEMMshElementForLF.end(), index) != (((this)->pcs)->_problem)->OGSHE_FEMMshElementForLF.end()) //Element ID for LF one defect domain, node [0] - 955KB mesh
	////////	{
	////////		int OGSHE_Position_In_Vector = std::find((((this)->pcs)->_problem)->OGSHE_FEMMshElementForLF.begin(), (((this)->pcs)->_problem)->OGSHE_FEMMshElementForLF.end(), index) - (((this)->pcs)->_problem)->OGSHE_FEMMshElementForLF.begin();
	////////		if (OGSHE_Position_In_Vector == (((this)->pcs)->_problem)->OGSHE_LF_ID[OGSHE_Position_In_Vector])
	////////		{
	////////			ST_PressureNodeIDs.push_back(nodes[(((this)->pcs)->_problem)->OGSHE_FEMMshNodePositionForLF[OGSHE_Position_In_Vector]]);//Node 0 of Element number = index
	////////			ST_Pressures.push_back(NodalVal0[(((this)->pcs)->_problem)->OGSHE_FEMMshNodePositionForLF[OGSHE_Position_In_Vector]]);//Pressure value of element = index
	////////			//cout << "Fill Pressure Vector with " << ST_Pressures[0] << endl;
	////////			//if (ST_Pressures[ST_Pressures.size() - 1] <= -2400)	//AP: Exfiltration is constant below this threshold value
	////////			//{
	////////			//	ST_Pressures[ST_Pressures.size() - 1] = -2400; //AP if node pressure below threshold, exfiltration as converged in leakage function
	////////			//}
	////////		}
	////////		else
	////////		{
	////////			cout << "Error: check filling of pressure vector for LF" << endl;
	////////		}
	////////	}
	////////}
	//}
