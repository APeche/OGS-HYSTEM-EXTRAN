...
//							if (ExtranCouplingVariable == 1)//; && (bc_eqs_index == 91);			//AP_2.6.15	
//								{	
									//for (bc_eqs_index = 23; bc_eqs_index < 92; bc_eqs_index++)
							//{
//----------------------------------------------------------------------------------------------------------------------------------
							//AP: Piece of Code for waterlevel relative to pipe geodetic height (circular function) 
							//double pipe_radius = .15;
							//int pipe_dis = 16;         //pipe discretization
							//double pipe_dx = pipe_radius / pipe_dis;      //distance between discretized points
							//vector<double> pipe_edge_points(pipe_dis + 1);  //empty vector pipe knots in x-dir
							//vector<double> pipe_fun(pipe_dis + 1);      //empty vector for pipe function (for knots in x,y-dir)
							//vector<double> pipe_bc_height(pipe_dis);           //empty vector for mean boundary height
							//for (int a_pipe = 0; a_pipe <= pipe_dis; a_pipe++)
							//{
							//	pipe_edge_points[a_pipe] = a_pipe*pipe_dx;        //write pipe knots in x-dir
							//	pipe_fun[a_pipe] = pipe_radius + (-sqrt(pow(pipe_radius, 2) - pow(pipe_edge_points[a_pipe], 2)));     //write pipe knots in x- and y-dir
							//}
							//for (int a_pipe2 = 1; a_pipe2 <= pipe_dis; a_pipe2++)
							//{
							//	pipe_bc_height[a_pipe2] = 0.5*(pipe_fun[a_pipe2] + pipe_fun[a_pipe2 - 1]);
							//}
							//if (m_bc->getGeoName() == "PIPE")
							//		{
							//			bc_value = gravity_constant * MFPGetNodeValue(m_bc_node->msh_node_number, "DENSITY", 0) *
							//				(_problem->PipeWaterLevelNull - (this->m_msh->nod_vector[m_bc_node->msh_node_number]->Z() - 3.89));	//example for Arne's test domain
							bc_value = time_fac * fac * m_bc_node->node_value;
//----------------------------------------------------------------------------------------------------------------------------------
							if (_problem->OGSHE == 0 && _problem->OGSHECouplingBC == "$DIRICHLET" && std::find(_problem->PipeName.begin(), _problem->PipeName.end(), m_bc->getGeoName()) != _problem->PipeName.end())
							{ 
									int OGSHE_Position_In_Vector = std::find(_problem->PipeName.begin(), _problem->PipeName.end(), m_bc->getGeoName()) - _problem->PipeName.begin();
									bc_value = time_fac * fac * (gravity_constant * MFPGetNodeValue(m_bc_node->msh_node_number, "DENSITY", 0) *
										_problem->PipeWaterLevel[OGSHE_Position_In_Vector]);
							}
//////////							if (m_bc->getGeoName() == "PIPE")
//////////									{
//////////										bc_value = (gravity_constant * MFPGetNodeValue(m_bc_node->msh_node_number, "DENSITY", 0) *
//////////											(_problem->PipeWaterLevelDatumn[1]-3));// - (.056*gravity_constant * MFPGetNodeValue(m_bc_node->msh_node_number, "DENSITY", 0));
//////////										/* 									bc_value = fac * gravity_constant * MFPGetNodeValue(m_bc_node->msh_node_number, "DENSITY", 0) *
//////////										(time_fac * (_problem->PipeWaterLevelNull - (.775 - this->m_msh->nod_vector[m_bc_node->msh_node_number]->Z())));*/ //value from named pipes here/identified time step variable for Extran 
//////////							//cout << "bc_value at Pipe Null is: " << bc_value << "\n";
////////////----------------------------------------------------------------------------------------------------------------------------------
//////////										//Might be used for later leakage flow rate calculation
//////////										//
//////////										//cout << "Density is: " << MFPGetNodeValue(m_bc_node->msh_node_number, "DENSITY", 0) << "\n";
//////////										//cout << "Node elevation: " << this->m_msh->nod_vector[m_bc_node->msh_node_number]->Z() << "\n";
//////////										//cout << "Permeability is: " << MFPGetNodeValue(m_bc_node->msh_node_number, "PERMEABILITY_TENSOR", 0) << "\n";
////////////----------------------------------------------------------------------------------------------------------------------------------
//////////									}
							//else if (m_bc->getGeoName() == "PIPE2")
							//{
							//	bc_value = gravity_constant * MFPGetNodeValue(m_bc_node->msh_node_number, "DENSITY", 0) *
							//		_problem->PipeWaterLevelOne;
							//}
							//else if (m_bc->getGeoName() == "PIPE3")
							//{
							//	bc_value = gravity_constant * MFPGetNodeValue(m_bc_node->msh_node_number, "DENSITY", 0) *
							//		_problem->PipeWaterLevelOne;
							//}
...
**************************************************************************/
	void CRFProcess::IncorporateSourceTerms(const int rank)
	{
		...
		int curve, valid = 0;
		long msh_node, shift;
		//AP OGSHE Variablen Initialisierung
		//-------------------------------------
		int ogshe_k = 0;
		vector <double> zufluss_temp;
		ofstream Leakagefile;
		vector <vector <double>> OGSHE_output;
		int HE_ST_id = 0;
		//-------------------------------------
...
//#####################
		...
		for (i = begin; i < end; i++)
		{
			...
			//----------------------------------------------------------------------------------------
			// AP HE-water level dependent ST for GEO_TYPE POLYLINE
			//----------------------------------------------------------------------------------------
				if (((((this)->fem)->pcs)->_problem)->OGSHE == 0 && ((((this)->fem)->pcs)->_problem)->OGSHECouplingBC == "$LEAKAGE_FUNCTION")
				{
					if (std::find(((((this)->fem)->pcs)->_problem)->PipeName.begin(), ((((this)->fem)->pcs)->_problem)->PipeName.end(), (m_st)->geo_name) != ((((this)->fem)->pcs)->_problem)->PipeName.end())
					{
						double P_temp;
						double P_temp2;
						...
						{
							if (GetNodeValue(msh_node, 1) < -2400 || *((this)->eqs_x) < -2400) //leakage flow below this pressure only depends on pipe water level
							...
							if (abs(this->_problem->PipeWaterLevel[ogshe_k]) <= .001) // If P < 0 && H_{PW}=0, Q_{leak} = 0; 
							{
								value = 0;
							}
							else
							{
								...
								value = ((((this)->fem)->pcs)->_problem)->Defect_Area[ogshe_k] * ((((this)->fem)->pcs)->_problem)->OGSHEPipeLenght[ogshe_k] * (1 / .000225) *
									(((((this)->fem)->pcs)->_problem)->a00 + ((((this)->fem)->pcs)->_problem)->a10 * P_temp + ((((this)->fem)->pcs)->_problem)->a01 * this->_problem->PipeWaterLevel[ogshe_k] +
									((((this)->fem)->pcs)->_problem)->a20 * pow(P_temp, 2) +
									((((this)->fem)->pcs)->_problem)->a11 * P_temp * this->_problem->PipeWaterLevel[ogshe_k] + ((((this)->fem)->pcs)->_problem)->a02 * pow(this->_problem->PipeWaterLevel[ogshe_k], 2) +
									((((this)->fem)->pcs)->_problem)->a30 * pow(P_temp, 3) + ((((this)->fem)->pcs)->_problem)->a21 * pow(P_temp, 2) * this->_problem->PipeWaterLevel[ogshe_k] +
									...
									((((this)->fem)->pcs)->_problem)->a40 * pow(P_temp, 4) + ((((this)->fem)->pcs)->_problem)->a31 * pow(P_temp, 3) * this->_problem->PipeWaterLevel[ogshe_k] +
									((((this)->fem)->pcs)->_problem)->a22 * pow(P_temp, 2) * pow(this->_problem->PipeWaterLevel[ogshe_k], 2) +
									((((this)->fem)->pcs)->_problem)->a13 * P_temp * pow(this->_problem->PipeWaterLevel[ogshe_k], 3)); //overwrite (ST-) value with value from leakage function
							}
						...
						else // GW level above pipe water level - Infiltration - Darcy with local hydraulic conductivity
						{
								value =  ((((this)->fem)->pcs)->_problem)->Defect_Area[ogshe_k] * ((((this)->fem)->MediaProp)->local_permeability *
								((this)->fem)->rhow * gravity_constant / fem->FluidProp->Viscosity()) *
								((this->_problem->PipeWaterLevel[ogshe_k] + ((((this)->fem)->pcs)->_problem)->Colmation_Layer_Thickness) - ((GetNodeValue(msh_node, 1) / (((this)->fem)->rhow * gravity_constant))) / ((((this)->fem)->pcs)->_problem)->Colmation_Layer_Thickness); // To DO: replace .0009 with $Pipe_Defect_Area and .02 with $Colmation_Layer_Thickness
						}
						if (i == 0 && (Tim->step_current - 1) % ((((this)->fem)->pcs)->_problem)->LF_output_steps == 0) //output every 60 (dt = 1s) or 120 (dt = 2s) sec
						{
							...
							Leakagefile << "node_id" << " " << "he_water_level_[m]" << " " << "st_node_pressure_[Pa]" << " " << "leakage_flow_[m^3/s]" << endl;
						}
						// ap data output
				...
				//----------------------------------------------------------------------------------------
				//--------------------------------------------------------------------
				// AP: OGS-HE: ST leakage function
				// AP HE-water level dependent ST for GEO_TYPE POINT
				// Overwrite ST NODE with Leakage fun value
				//-------------------------------------------------------------------- 
				//cout << "start leakage function calc" << endl;
				//for (int k = 0; k < fem->ST_PressureNodeIDs.size() ; k++)
				//{
				//	cout << "ST node ID: " << fem->ST_PressureNodeIDs[k] << endl;
				//	if (msh_node == fem->ST_PressureNodeIDs[k])
				//	{
				//		value = a00 + a10 * fem->ST_Pressures[k] + a01 * this->_problem->PipeWaterLevel[2] + a20 * pow(fem->ST_Pressures[k], 2) +
				//			a11 * fem->ST_Pressures[k] * this->_problem->PipeWaterLevel[2] + a02 * pow(this->_problem->PipeWaterLevel[2], 2) +
				//			a30 * pow(fem->ST_Pressures[k], 3) + a21 * pow(fem->ST_Pressures[k], 2) * this->_problem->PipeWaterLevel[2] +
				//			a12 * fem->ST_Pressures[k] * pow(this->_problem->PipeWaterLevel[2], 2) + a03 * pow(this->_problem->PipeWaterLevel[2], 3) +
				//			a40 * pow(fem->ST_Pressures[k], 4) + a31 * pow(fem->ST_Pressures[k], 3) * this->_problem->PipeWaterLevel[2] +
				//			a22 * pow(fem->ST_Pressures[k], 2) * pow(this->_problem->PipeWaterLevel[2], 2) +
				//			a13 * fem->ST_Pressures[k] * pow(this->_problem->PipeWaterLevel[2], 3);
				//		//value *= 2.48;	//AP Temporary factor for quadratic standard failure (Karpf, 2012)  
				//			// AP OGSHE Output of leakage rates at each timestep 
				//			//-------------------------------------------------------------------------------------
				//			// AP Initialize temporal output vector - output node id, nodal pressure, HE water level, Q_leak 
				//			double OGSHE_output_temp[] = { msh_node, fem->ST_Pressures[OGSHE_k], this->_problem->PipeWaterLevel[2], value };
				//			// AP Initialize temporal output file
				//			if (i == 0)
				//			{
				//				// AP OGSHE New output each timestep 
				//				std::stringstream ss;	// AP Initialize empty stringstream
				//				ss << "LeakageFlow_time_" << Tim->step_current-1 << ".csv"; //Fill stringstream
				//				LeakageFile.open(ss.str().c_str());	//Name file after stringstream 
				//				LeakageFile << "OGSHE_temporal_output" << endl; //Temporal Output File Header
				//				LeakageFile << "Nodes per ST Line Element: " << m_st->st_node_ids.size() << endl;
				...
				//			}
				//				// AP data output
				//				LeakageFile << OGSHE_output_temp[0] << " " << this->_problem->PipeWaterLevel[2] << " " << fem->ST_Pressures[OGSHE_k] << " " << value << endl;

				//		//this->_problem->inflow[i] = value; //AP inflow for HE ST 
				//	}
				//}
				//--------------------------------------------------------------------
