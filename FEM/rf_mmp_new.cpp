...
{
	double kr = 0.0, sl, se, slr, slm, m, b, kr2, p1, p2, p3, p4, p5, p6, p7, p8, p9;
	int model, gueltig;
	...
	switch(model)
	{
		...
		case 4: // 2-phase VAN GENUCHTEN/MUALEM --> WETTING
			...
			// -------------------------------------------------------------------
			// Für Validierung von OGS-HE anhand physikalischem Experiment von Skaggs (1970)
			// -------------------------------------------------------------------
			// AP 5/16 From experimental data derived polygon 9th order for relative conductivity function, because classic van Genuchten doesn't work
			//p1 = 5.89e6;
			//p2 = -7357603.3;
			//p3 = 3.674331610501483e+06;
			//p4 = -9.451100906881209e+05;
			//p5 = 1.351449330798654e+05;
			//p6 = -1.081372695218048e+04;
			//p7 = 4.639667279062906e+02;
			//p8 = -9.433955511377102;
			//p9 = 0.072;
			//kr = p1 * pow((se*.35 / slm), 8) + p2*pow((se*.35 / slm), 7) + p3*pow((se*.35 / slm), 6) + p4 *pow((se*.35 / slm), 5)
			//	+ p5*pow((se*.35 / slm), 4) + p6*pow((se*.35 / slm), 3) + p7* pow((se*.35 / slm), 2) + p8 *pow((se*.35 / slm), 1)
			//	+ p9 *pow((se*.35 / slm), 0);
			//if (se > .866) // this part didn't work because the conductivity is at its max at this threshold
			//	kr = 1;
			// -------------------------------------------------------------------
			if(kr < minimum_relative_permeability)
				kr = minimum_relative_permeability;
