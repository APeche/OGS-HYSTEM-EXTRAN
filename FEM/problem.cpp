...
/* HE Coupling: for reading from OpenGeoSysKopplung.xml */
#include "Pipe.h"							//AP_7.6.15: Pipe header file for Extran coupling
#include <conio.h>							//AP_15.6.15: input/output header file for Extran coupling
#include <fstream> 
#include <string.h>
#include <stdio.h>
#include <string>
...
#include <unistd.h>
#endif
//#define access    _access_s					// AP:04.2017
//
#ifdef CHEMAPP
#include "eqlink.h"
#endif
...
// AP This function checks if OGS-HE files exist?
//bool CheckIf_HE_FileExists(const std::string &Filename)
//{
...
void Problem::Euler_TimeDiscretize()
{
	...
	/*HE COUPLING - Initialisation*/
	std::cout << "This is OGSHE" << std::endl;
	checkIfOGSHECouplingIsActive();
	COleDateTime zeitpunkt;
	//CPipe *pipe = nullptr;
	if (OGSHE == 0)
	{
		readNamedPipeNameForHECoupling();
		readHEFile();
		InitiateOGSHECoupling();

		//char buffer[255];
		//printf("Initiate connecting to HE\n");
		//pipe = new CPipe();
		//std::cout << "pipe is " << pipe << std::endl;
		//printf("NamedPipes connected\n");
		//if (!pipe->VerbindePipes(Name_Pipe_Write.c_str(), Name_Pipe_Read.c_str())) // Names of pipes as LPCTSTR [.c_str()] according to *.xml file (to do: implement read name of pipes from *.xml file)
		//{
		//	printf("NamedPipes not connected --> Check OpenGeoSysKopplung.xml. Press any key to end program.\n");
		//	_getch();
		//	delete pipe;
		//	//return 1;
		//}
		...
		//pipe->PutD(exchange_time_step_OGSHE);
		//std::cout << "Austauschzeitschritt: " << exchange_time_step_OGSHE << "\n";
		//pipe->GetTime(zeitpunkt);
		//std::cout << "Zeitpunkt: " << zeitpunkt << "\n";
		...
		//}
		//bool Extran_ok = true;
		//printf("Extran coupling initiated \n");
	}
	//std::cout << "pipe is " << pipe << std::endl;
...
//
		...
		//}
			if (OGSHE == 0)
			{
				pipe->GetTime(zeitpunkt); // get zeitpunkt before going into function
				std::cout << "Zeitpunkt: " << zeitpunkt << "\n";
				OGSHEDataTransfer();
			}
			...
			//}
				...
				////////////-------------------------------------------------------------------------------------//
				////////////Extran coupling - parameter exchange for every time step
				////////////-------------------------------------------------------------------------------------//
				//////////std::cout << "zeitpunkt Objekt erzeugt" << std::endl;
				////////pipe->GetTime(zeitpunkt);
				////////std::cout << "Zeitpunkt: " << zeitpunkt << "\n";
				////////std::cout << "------------------------------------------------------- \n";
				////////////Read Manhole geodaetic heights at initial time step
				////////if (exchange_time_step_OGSHE == 1)
				////////{
				////////	// AP ISUville 2DT PipeDatumnHeight = { 247.982, 246.452, 244.197, 251.747, 242.332, 241.437 };
				////////	PipeDatumnHeight = { 3, 2, 1, 0 };
				////////}
				//////////AP erase ManholeWaterLevel- and PipeWaterLevel vector before every time step calculation
				////////PipeWaterLevelDatumn.erase(PipeWaterLevelDatumn.begin(), PipeWaterLevelDatumn.end());
				////////PipeWaterLevel.erase(PipeWaterLevel.begin(), PipeWaterLevel.end());
				//////////loop in order to substract reference height (msl) from ExtranWaterLevel
				////////for (long i = 0; i < anzahlSchaechte; i++)
				////////{
				////////	std::cout << "get wasserstand" << std::endl;

				////////	pipe->GetD(wasserstand);  // AP wasserstand ist wasserstand in Schacht
				////////	std::cout << "got wasserstand" << std::endl;
				...
				////////for (long i = 0; i < anzahlHaltungen; i++)
				////////{
				////////	pipe->PutD(zufluss[i] * (-1));
				////////	std::cout << "Pipe No : " << i << " Zufluss : " << zufluss[i] * (-1) << "\n";
				////////}
				////////std::cout << "------------------------------------------------------- \n";
				////////long couplingstatus;
		...
		if(CouplingLoop())
		{
			...
			//if (current_time == end_time)
			//{
			//	pipe->PutI(couplingstatus = 1);
			//	delete pipe;	//AP 23.6.15 delete Extran NamedPipe
			//}
			PostCouplingLoop();
			if(print_result)
			{
...
OGS-HE Coupling:
04/2017 AP
This function checks if input files ('OpenGeoSysKopplung.xml', *.he) exist
**************************************************************************/
void Problem::checkIfOGSHECouplingIsActive()
{
	filenameOGSHEXML = "OpenGeoSysKopplung.xml";
	filenameOGSHE_HE_File = "Coupling2HE.he";
	std::ifstream ifile(filenameOGSHEXML.c_str());
	std::ifstream jfile(filenameOGSHE_HE_File.c_str());
	if (!ifile.is_open() && !jfile.is_open()) {
		std::cerr << "HE Coupling - no such files: OpenGeoSysKopplung.xml or *.he" << std::endl;
		OGSHE = 1; //coupling bool 0->Coupling, 1->No Coupling
	}
	else
	{
		std::cerr << "Found required input files for coupling to HE. Initiate coupling." << std::endl;
		OGSHE = 0;
	}
}
...
OGS-HE Coupling:
04/2017 AP
This function reads the name of the NamedPipe assigned in 'OpenGeoSysKopplung.xml'
**************************************************************************/
void Problem::readNamedPipeNameForHECoupling()
{
	
	std::cout << "Found OpenGeoSysKopplung.xml" << std::endl;
	std::string OGSHE_pre1 = "\\\\.\\pipe\\"; //path to append names of pipes with
	std::string OGSHE_pre2 = "\\\\.\\pipe\\"; //path to append names of pipes with
	std::ifstream in("OpenGeoSysKopplung.xml"); //Input OpenGeoSysKopplung.xml
	in.ignore(200, '\n');	//Jump lines to names of named pipes
	in.ignore(200, '\n');
	in.ignore(200, '\n');
	in.ignore(200, '\n');
	in.ignore(200, '\n');
	std::string PipeReadTemp;	//Initiate string for whole line from *.xml
	in >> PipeReadTemp;
	char R_start = PipeReadTemp.find('GW');	//Find start of NamedPipe name
	char R_end = PipeReadTemp.find('</');	//Find end of NamedPipe name
	std::string Name_Pipe_Read_end = PipeReadTemp.substr(R_start - 1, R_end - R_start); //Cut name (temporary - improve this)
	Name_Pipe_Read = OGSHE_pre1.append(Name_Pipe_Read_end);
	std::string PipeWriteTemp;
	in >> PipeWriteTemp;
	char W_start = PipeWriteTemp.find('GW');
	char W_end = PipeWriteTemp.find('</');
	std::string Name_Pipe_Write_end = PipeWriteTemp.substr(W_start - 1, W_end - W_start);
	Name_Pipe_Write = OGSHE_pre2.append(Name_Pipe_Write_end);
	std::cout << Name_Pipe_Read << std::endl;
	std::cout << Name_Pipe_Write << std::endl;
...
OGS-HE Coupling:
04/2017 AP
This function reads the data of the HYSTEM-EXTRAN model setup from the *.he file.

**************************************************************************/
void Problem::readHEFile()
{
	std::vector<std::string> DataArray;
	// Create input stream.
	std::ifstream myfile("Coupling2.he");
	copy(std::istream_iterator<std::string>(myfile),
		std::istream_iterator<std::string>(),
		back_inserter(DataArray));
	OGSHECouplingBC = DataArray.at(1);
	std::cout << "HE coupling based on " << OGSHECouplingBC << " BCs" << "\n";
	NoPipes = atoi(DataArray.at(5).c_str()); //int defining number of pipes
	exchange_time_step_OGSHE = atof(DataArray.at(3).c_str());
	std::cout << "OGSHE exchange time step: " << exchange_time_step_OGSHE << " s" << std::endl;
	if (DataArray.at(1) == "$DIRICHLET")
	{
		for (long i = 0; i < NoPipes; i++)//continue here putting stuff from *.he in vectors
		{
			PipeName.push_back(DataArray.at(7 + (i * 4)).c_str()); //these variables give information about the HE model (pipe name, connecting manholes, pipe height over datumn) 
			UpstreamManhole.push_back(atoi(DataArray.at(8 + (i * 4)).c_str()));
			DownstreamManhole.push_back(atoi(DataArray.at(9 + (i * 4)).c_str()));
			PipeDatumn.push_back(atof(DataArray.at(10 + (i * 4)).c_str()));
		}
		try {
			std::cout << "Number of Pipes in *.he file: " << DataArray.at(5) << "\n";
		}
		catch (...) {
			...
			std::cout << "Data in *.he file unavailable\n";
		}
	}
	...
	{
		std::cout << "Error: no such keyword " << DataArray.at(5) << " accepted. Use $DIRICHLET or $LEAKAGE_FUNCTION instead." << std::endl;
	}
...
OGS-HE Coupling:
05/2017 AP
This function connects the NamedPipes and does the initial data transfer between HE and OGS.

**************************************************************************/
void Problem::InitiateOGSHECoupling()
{
	COleDateTime zeitpunkt;
	char buffer[255];
	printf("Initiate connecting to HE\n");
	...
	std::ofstream sas("HEPipeNames.txt");
	for (long i = 0; i < anzahlHaltungen; i++)
	{
		pipe->GetS(buffer);
		HEPipeNames.push_back(buffer);
		sas << i << "   " << HEPipeNames[i] << std::endl;;
	}
	sas.close();
	pipe->GetI(anzahlSchaechte);
	printf("Number of manholes: %d\n", anzahlSchaechte);
	std::ofstream ss("HEManholeNames.txt");
	for (long i = 0; i < anzahlSchaechte; i++)
	{
		pipe->GetS(buffer);
		HEManholeNames.push_back(buffer);
		ss <<i<<"   "<< HEManholeNames[i] << std::endl;;
	}
	ss.close();
	printf("Extran coupling initiated \n");
}
...
OGS-HE Coupling:
05/2017 AP
This function does the data transfer between HE and OGS.

**************************************************************************/
void Problem::OGSHEDataTransfer()
{
	//AP erase ManholeWaterLevel- and PipeWaterLevel vector before every time step calculation
	PipeWaterLevelDatumn.erase(PipeWaterLevelDatumn.begin(), PipeWaterLevelDatumn.end());
	PipeWaterLevel.erase(PipeWaterLevel.begin(), PipeWaterLevel.end());
	...
	if (anzahlHaltungen != PipeDatumn.size())
	{
		if (aktueller_zeitschritt == 1)
		{
			std::cout << "Warning: the number of pipes does not correspond to number of pipes in HE file" << std::endl;
		}
	...
	else
	{
		...
		if (aktueller_zeitschritt == 1)
		{
			std::vector<double> vector(anzahlHaltungen, 0.0);
			zufluss = vector;
		}
		for (long i = 0; i < anzahlHaltungen; i++)
		{
			pipe->PutD(zufluss[i] * (-1));
			//std::cout << "Pipe No : " << i << " Leakage : " << zufluss[i] * (-1) << "m^3/s" << "\n";
		}
		std::cout << "------------------------------------------------------- \n";
		long couplingstatus;
		if (current_time < end_time)
